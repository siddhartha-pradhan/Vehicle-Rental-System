    ...
    public IActionResult Index()
    {
        ...
        var vehicles = (from vehicle in _vehicleService.GetAllVehicles().Where(x => x.IsAvailable)
                        select new VehiclesViewModel
                        {
                            ...
                            ((_offerService.RetrieveOffer(vehicle.OfferId).EndDate > DateTime.Now && _offerService.RetrieveOffer(vehicle.OfferId).StartDate < DateTime.Now)
                            ? $"{_offerService.RetrieveOffer(vehicle.OfferId).Discount}% Offer" : "No offer") : "No offer",
